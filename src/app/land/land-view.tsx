...
export default function LandView() {
  ...
  const { lands, loading, error, minting, mintDemo, refresh, canTransact } = useLand();
  ...
  return (
    <Box sx={{ display:"flex", flexDirection:"column", gap:2 }}>
      ...
      <Alert severity="info">
        Mint a demo Land NFT via <b>Panna gasless</b>. Backend must expose <code>POST {BACKEND_URL}/lands/pin</code>.
      </Alert>

      <Button variant="contained" disabled={!canTransact || minting} onClick={mintDemo} sx={{ width:{ xs:"100%", sm:"fit-content"} }}>
        {minting ? <CircularProgress size={16} sx={{ mr:1 }} /> : null}
        {minting ? "Submitting mint..." : "Mint Land NFT (Gasless Demo)"}
      </Button>

      <Typography variant="body2" color="text.secondary">
        Owner: {address ? `${address.slice(0,6)}...${address.slice(-4)}` : "Not connected"}
      </Typography>
